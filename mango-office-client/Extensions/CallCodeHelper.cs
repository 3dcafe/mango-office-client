...
namespace MangoOfficeClient.Extensions
{
    public class CallCodeHelper
    {
        ...
        {
            string mainCodes = @"1000 Действие успешно выполнено
...
1234 Сессия завершена администратором, рекомендовано оставаться воффлайн
...
5004 Проблемы доступа к базе данных коммутатора: не удалось подключиться к базе данных или в результате обработки запроса на манипуляцию с даными(чтение / вставка / удаление / и так далее) в базе данных выдано сообщение об ошибке.
...
6102 Превышено допустимое число страниц(30)";
            var string_codes = mainCodes.Split('\n');
            foreach (var code_string in string_codes)
            {
                var words = code_string.Split(' ');
                var _code = words.FirstOrDefault();
                var description = String.Join(" ", words.Skip(1)).RemoveSpecialCharacters();
                if (int.TryParse(_code, out int c))
                {
                    codes.Add(c, description);
                }
            }
        }
        #endregion
